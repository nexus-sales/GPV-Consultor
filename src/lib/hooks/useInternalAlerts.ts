  ...
  function runChecks() {
    ...
    if (criticalVisits.length > 0) {
      const names = criticalVisits
        ...
        .join(', ')
      const extra = criticalVisits.length > 2 ? ` y ${criticalVisits.length - 2} más` : ''
      toast.error(`${criticalVisits.length} distribuidor${criticalVisits.length > 1 ? 'es' : ''} sin visita`, {
      ...
      const names = warningVisits
        ...
        .map((a) => a.distributorName)
        .join(', ')
      const extra = warningVisits.length > 2 ? ` y ${warningVisits.length - 2} más` : ''
    ...
    const urgentTasks = tasks.filter(
      (t) => t.status === 'pending' && t.dueDate && t.dueDate.slice(0, 10) <= today
    )
    if (urgentTasks.length > 0) {
      const overdue = urgentTasks.filter((t) => t.dueDate && t.dueDate.slice(0, 10) < today)
      const dueToday = urgentTasks.filter((t) => t.dueDate && t.dueDate.slice(0, 10) === today)
      const parts: string[] = []
    ...
    if (staleCandidates.length > 0) {
      const names = staleCandidates
        ...
        .map((c) => c.name)
        .join(', ')
      const extra = staleCandidates.length > 2 ? ` y ${staleCandidates.length - 2} más` : ''
      toast.info(`${staleCandidates.length} candidato${staleCandidates.length > 1 ? 's' : ''} sin contacto reciente`, {
