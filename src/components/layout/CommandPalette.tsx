...
export const CommandPalette: React.FC = () => {
  ...
  const commands = useMemo((): Command[] => {
    const staticCommands: Command[] = [
      { id: 'nav-dash', title: 'Ir a Dashboard', description: 'Vista general ejecutiva', icon: HomeIcon, category: 'Navegación', onSelect: () => navigate('/') },
      { id: 'nav-kanban', title: 'Ir a Pipeline', description: 'Flujo de ventas Kanban', icon: ChartBarIcon, category: 'Navegación', onSelect: () => navigate('/kanban') },
      { id: 'nav-dist', title: 'Ir a Distribuidores', description: 'Red de distribución comercial', icon: UsersIcon, category: 'Navegación', onSelect: () => navigate('/distributors') },
      { id: 'nav-leads', title: 'Ir a Leads', description: 'Prospectos Google Maps', icon: IdentificationIcon, category: 'Navegación', onSelect: () => navigate('/leads') },
      { id: 'nav-visits', title: 'Ir a Visitas', description: 'Calendario de visitas', icon: CalendarIcon, category: 'Navegación', onSelect: () => navigate('/visits') },
      { id: 'nav-sales', title: 'Ir a Pedidos', description: 'Control de activaciones', icon: ShoppingBagIcon, category: 'Navegación', onSelect: () => navigate('/sales') },
      { id: 'nav-cand', title: 'Ir a Candidatos', description: 'Reclutamiento activo', icon: UserGroupIcon, category: 'Navegación', onSelect: () => navigate('/candidates') },
      { id: 'nav-import', title: 'Ir a Importar Datos', description: 'Carga masiva Excel/CSV', icon: ArrowUpTrayIcon, category: 'Navegación', onSelect: () => navigate('/import') },
      { id: 'nav-report', title: 'Ir a Reportes', description: 'Métricas semanales', icon: DocumentTextIcon, category: 'Navegación', onSelect: () => navigate('/reports') },
      { id: 'nav-settings', title: 'Ir a Configuración', description: 'Preferencias del sistema', icon: CogIcon, category: 'Navegación', onSelect: () => navigate('/settings') },
      
      { id: 'action-theme', title: `Cambiar a modo ${isDark ? 'claro' : 'oscuro'}`, description: 'Ajustar apariencia visual', icon: isDark ? SunIcon : MoonIcon, category: 'Acciones', onSelect: () => toggle() },
    ]
    ...
    if (query.length > 1) {
      ...
      // Distribuidores
      distributors
        .filter(d => d.name.toLowerCase().includes(lowerQuery) || d.code.toLowerCase().includes(lowerQuery))
        .slice(0, 5)
        .forEach(d => entityResults.push({
          id: `dist-${d.id}`,
          title: d.name,
      ...
      // Leads
      leads
        .filter(l => l.nombre.toLowerCase().includes(lowerQuery) || l.ciudad?.toLowerCase().includes(lowerQuery))
        .slice(0, 5)
        .forEach(l => entityResults.push({
          id: `lead-${l.id}`,
          title: l.nombre,
      ...
      <div className="relative w-full max-w-2xl bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 pointer-events-auto overflow-hidden animate-slide-up flex flex-col max-h-[70vh]">
        ...
        <div className="relative flex items-center px-6 py-5 border-b border-slate-100 dark:border-slate-800">
          ...
          <div className="flex items-center gap-1.5 ml-4">
             <kbd className="hidden sm:inline-flex items-center justify-center h-6 w-6 rounded bg-slate-100 dark:bg-slate-800 border-b-2 border-slate-200 dark:border-black text-[10px] font-bold text-slate-500">ESC</kbd>
          </div>
        </div>
        ...
        >
          {commands.length === 0 ? (
            <div className="py-12 text-center">
              ...
              <p className="text-slate-400 font-medium tracking-tight">No se encontraron comandos o resultados</p>
            </div>
          ) : (
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <p className={`font-bold text-sm ${isSelected ? 'text-white' : 'text-slate-900 dark:text-slate-100'}`}>
                              {command.title}
                            </p>
                            {command.description && (
                          ...
                          {isSelected && (
                            <div className="flex items-center gap-2">
                               <span className="text-[10px] font-black text-white/60 uppercase tracking-widest">ENTER</span>
        ...
        <div className="px-6 py-4 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-[10px] text-slate-400 font-bold uppercase tracking-wider">
           <div className="flex items-center gap-4">
             <span className="flex items-center gap-1.5">
               <kbd className="px-1.5 py-0.5 rounded bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400">↑↓</kbd> Navegar
             </span>
             <span className="flex items-center gap-1.5">
               <kbd className="px-1.5 py-0.5 rounded bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400">↵</kbd> Seleccionar
             </span>
           </div>
