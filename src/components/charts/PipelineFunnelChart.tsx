...
const STAGE_COLORS: Record<string, { bar: string; text: string; badge: string }> = {
  new: {
  ...
  return (
    <div className="rounded-[20px] bg-white/70 dark:bg-slate-900/60 backdrop-blur-xl border border-white dark:border-slate-800 shadow-lg shadow-slate-200/40 dark:shadow-none p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-sm font-black text-slate-900 dark:text-white">Conversión del Pipeline</h3>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{total} candidatos activos</p>
        </div>
        <button
      ...
      {total === 0 ? (
        <p className="text-sm text-slate-400 italic text-center py-6">Sin candidatos en el pipeline</p>
      ) : (
              ...
              <div key={stage.id}>
                ...
                {rate !== null && i > 0 && (
                  <div className="flex items-center gap-2 py-1 pl-1">
                    ...
                    <span className={`text-[10px] font-bold ${rate >= 50 ? 'text-emerald-600 dark:text-emerald-400' : rate >= 25 ? 'text-amber-600 dark:text-amber-400' : 'text-rose-600 dark:text-rose-400'}`}>
                      {rate}% conversión
                    </span>
                  </div>
                )}
                ...
                >
                  <div className="flex items-center justify-between mb-1.5">
                    <span className={`text-xs font-bold ${colors.text}`}>{stage.label}</span>
      ...
      {(() => {
        ...
        return (
          <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
            <span className="text-xs text-slate-400 font-medium">Rechazados</span>
